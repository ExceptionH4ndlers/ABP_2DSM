...
export function createStationPopupHTML(point: MapPoint): string {
  ...
  return `<div style="min-width: 200px; width: max-content; max-width: 600px; background: white; border-radius: 20px; box-shadow: 0 8px 24px rgba(0, 0, 0, 0.15), 0 2px 8px rgba(0, 0, 0, 0.1); border: 2px solid #e2e8f0; margin: 0; padding: 0; position: relative; overflow: hidden;">
      <div style="background: ${getGradient(point.type)}; padding: 16px 50px 16px 18px; color: white; display: flex; align-items: flex-start; gap: 12px; position: relative; border-radius: 20px 20px 0 0;">
        <button onclick="var p=this.closest('.leaflet-popup');if(p&&window.L){var m=p._source;if(m)m.closePopup();else window.L.DomEvent.getPropagationPath({target:p}).find(function(e){return e.__leaflet})?.closePopup();}" onmouseover="this.style.background='rgba(0,0,0,0.4)'" onmouseout="this.style.background='rgba(0,0,0,0.2)'" style="position: absolute; top: 12px; right: 12px; width: 28px; height: 28px; padding: 0; margin: 0; border: none; background: rgba(0, 0, 0, 0.2); border-radius: 50%; color: rgba(255, 255, 255, 0.9); font-size: 20px; font-weight: bold; cursor: pointer; display: flex; align-items: center; justify-content: center; z-index: 1000; transition: all 0.2s ease; line-height: 1; user-select: none;">×</button>
        ...
        <div style="flex: 1 1 auto; min-width: 0; max-width: none; overflow: visible; word-break: break-word; padding-right: 0;">
          <h3 style="font-size: 16px; font-weight: 700; margin: 0 0 4px 0; color: white; word-wrap: break-word; overflow-wrap: break-word; word-break: break-word; line-height: 1.4; white-space: normal; overflow: visible; text-overflow: clip; max-width: none;">${stationName}</h3>
